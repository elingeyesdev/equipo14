...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Avispáme — Inteligencia urbana para una ciudad más segura" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Avispáme — Inteligencia urbana en tiempo real" },
      {
...
const testimonials = [
  {
    quote:
      "Avispáme transformó la forma en que coordinamos con la comunidad. Antes tardábamos horas en saber qué pasaba. Ahora es instantáneo y verificado.",
  ...
  {
    quote:
      "Los datos que genera Avispáme son invaluables para nuestra analítica urbana. Calidad de datos superior a cualquier otra fuente que hayamos probado.",
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-background/85 backdrop-blur-xl shadow-[0_4px_30px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_30px_rgba(0,0,0,0.15)]">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        {/* Todo alineado a la derecha: links + Ingresar (botón destacado) */}
...
function Hero() {
  return (
    <section className="relative min-h-screen flex flex-col justify-center pt-24 pb-8 px-6 overflow-hidden bg-gradient-to-b from-background via-background to-muted/10">
      ...
      <div className="max-w-5xl mx-auto text-center relative z-10">
        <Reveal>
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-bold leading-[1.08] tracking-tight mb-4 text-foreground">
            Tu ciudad, más inteligente y segura.
          </h1>
        </Reveal>

        <Reveal delay={80}>
          <p className="text-sm md:text-base text-muted-foreground mb-6 max-w-lg mx-auto leading-relaxed">
        ...
        <Reveal delay={160}>
          <div className="relative max-w-sm mx-auto scale-90 sm:scale-95">
            <TiltPhone />
...
function About() {
  return (
    <section id="about" className="relative min-h-screen flex flex-col justify-center py-12 px-6 bg-primary text-primary-foreground overflow-hidden">
      <div className="max-w-5xl mx-auto w-full">
        <Reveal>
          <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-primary-foreground/70 mb-2">
            Cómo Avispáme potencia tu seguridad
          </p>
        </Reveal>

        <Reveal delay={80}>
          <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-bold leading-[1.15] tracking-tight mb-6 max-w-2xl text-balance">
            Avispáme es la plataforma que conecta ciudadanos con autoridades y organizaciones para prevenir incidentes y responder en tiempo real. Reporta, coordina y protege tu barrio.
          </h2>
        </Reveal>
        ...
        <Reveal delay={160}>
          <div className="relative max-w-2xl ml-auto">
            <div className="relative rounded-[1rem] overflow-hidden shadow-2xl bg-black/20 translate-x-2 sm:translate-x-4 lg:translate-x-6">
              <img
                src={liveMapImg}
                alt="Mapa en vivo de Avispáme mostrando incidentes y estaciones de emergencia"
                className="w-full aspect-video object-cover hover:scale-[1.02] transition-transform duration-700 max-h-[220px] sm:max-h-[260px]"
              />
            </div>
          </div>
        </Reveal>
...
function ExploreApp() {
  ...
  return (
    <section id="explore-app" className="relative min-h-screen flex flex-col justify-center py-12 px-6 bg-muted/10">
      <div className="max-w-5xl mx-auto w-full">
        {/* Header */}
        <div className="text-center mb-6 max-w-xl mx-auto">
          <Reveal>
            ...
            <h2 className="font-display text-2xl md:text-3xl font-bold tracking-tight mb-1 text-foreground">
              Explora la aplicación
            </h2>
            <p className="text-muted-foreground text-xs">
              Descubre las pantallas clave de Avispáme: reporta incidentes, visualiza riesgos en el mapa, consulta la actividad y recibe notificaciones en tiempo real.
            </p>
          </Reveal>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 items-center">
            ...
            <div className="space-y-1.5">
              {appScreens.map((s, idx) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl transition-all duration-300 border-none outline-none cursor-pointer ${
                    activeIndex === idx
                      ? "bg-card shadow-lg text-foreground animate-in fade-in zoom-in-95 duration-200"
                ...
                >
                  ...
                  <h4 className="font-display font-bold text-base mt-0.5 mb-0.5">{s.label}</h4>
                  {activeIndex === idx && (
          ...
          <div className="lg:col-span-6 flex justify-center order-1 lg:order-2">
            <Reveal className="w-full max-w-[160px] sm:max-w-[190px]">
              <div className="bg-neutral-950 p-1.5 rounded-[2rem] shadow-xl ring-4 ring-neutral-800/50 aspect-[9/19.5] flex flex-col overflow-hidden">
                {/* Notch */}
                <div className="w-12 h-3.5 bg-black rounded-full mx-auto mb-0.5 flex items-center justify-center shrink-0 z-10">
                  <span className="size-1 rounded-full bg-neutral-800" />
                </div>
                {/* Screen — actual screenshot image */}
                <div className="flex-1 overflow-hidden rounded-[1.6rem] relative bg-neutral-900">
                  {appScreens.map((s, idx) => (
                    <img
                      key={s.label}
                      src={s.img}
                      alt={`Pantalla de ${s.label} de la app Avispáme`}
                ...
                {/* Home indicator */}
                <div className="w-16 h-0.5 bg-neutral-600 rounded-full mx-auto mt-1 shrink-0" />
              </div>
            </Reveal>
          </div>
        </div>
...
function Testimonials() {
  ...
  return (
    <section id="testimonials" className="relative min-h-screen flex flex-col justify-center py-12 px-6 bg-muted/20">
      <div className="max-w-5xl mx-auto w-full">
        <div className="text-center mb-6 max-w-xl mx-auto">
          <Reveal>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-1">
        ...
        <div className="max-w-3xl mx-auto relative px-10">
          <div className="transition-all duration-500 text-center py-2">
            <p className="font-display text-base md:text-lg italic leading-relaxed text-foreground max-w-xl mx-auto">
              "{testimonials[idx].quote}"
            </p>
            <div className="mt-3 flex flex-col items-center">
              <div className="size-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-display font-bold text-sm mb-1.5">
                {testimonials[idx].initials}
              </div>
              <h4 className="font-bold text-xs text-foreground">{testimonials[idx].name}</h4>
              <p className="text-[10px] text-muted-foreground mt-0.5">{testimonials[idx].role}</p>
            </div>
          </div>
    ...
    <footer className="py-20 px-6 bg-muted/10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="pt-8 border-t border-muted/20 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-muted-foreground font-medium">
          <p>© 2035 por Avispáme. Potenciado y asegurado por Wix</p>
